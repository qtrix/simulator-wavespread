...
func (c *Charter) getPoint(sa *wavespread.Wavespread) (*Point, error) {
	...
	return &Point{
		...
		AnchorValue:         anchorLiq.Round(2),
		AnchorValueRegular:  sa.TotalAnchors.Round(2),
		SurferLiq:           surferLiq.Round(2),
		SurferValue:         surferLiq.Round(2),
		SurferValueRegular:  sa.TotalBalance.Sub(sa.TotalAnchors).Round(2),
